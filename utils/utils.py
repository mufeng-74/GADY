...
class GenFGANLoss(nn.Module):
    ...
    def forward(self, d_out, g_out):
        ...
        g_out[1] = g_out[1].reshape(g_out[1].shape[0],-1)
        g_out[2] = g_out[2].reshape(g_out[2].shape[0],-1)

        DL = 0
        epsilon = 1e-10 
        
        for i in range(len(g_out)):
          DL += torch.mean(torch.std(g_out[i],axis = 0)/(torch.mean(g_out[i],axis = 0)+epsilon))
        DL = DL/len(g_out)
